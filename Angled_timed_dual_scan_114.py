...
MOT_range = np.arange(-550e6/hertz_unit, 50e6/hertz_unit,float(25e6/hertz_unit))
slower_range = np.arange(-1200e6/hertz_unit, 500e6/hertz_unit,float(50e6/hertz_unit))
...
angle_range = np.linspace(0.,75e-3, 8)
# angle_range = [0]
# time_range = np.linspace(0e-4/time_unit,1e-3/time_unit,8)
time_range = [3.5e-3/time_unit]
...
def run_cap_vel_scan(i):
    ...
    cap_vels = []
    for t in time_range:
        cap_vels.append([[captureVelocityForEq_ranged(dMOT, dSlow, Hamiltonians[114], t, angle = i, lasers = MOT_and_Slow_Beams_timed) for dMOT in MOT_range] for dSlow in slower_range])
    return (i,cap_vels)
