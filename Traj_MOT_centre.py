...
def run():
    try:
        ...
        eq.set_initial_pop(np.array([0.5,0.5,0,0,0,0,0,0]))
    eq.set_initial_position_and_velocity(np.array([1e-3,0,0]),np.array([0,0,0]))
    return eq.evolve_motion([0., 25e-3/time_unit], events=[lost_condition,backwards_lost],
                      max_step=m_step,progress_bar=True)
r = run()

xlim = plt.xlim()
ylim = plt.ylim()

plt.plot(1e-2*r.r[0],velocity_unit*r.v[0], '-', color = "black" if isCaptured(r) < 1 else "green")
