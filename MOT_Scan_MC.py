...
if __name__ == "__main__":
    params = np.array(np.meshgrid(list(Hamiltonians.keys()), MOT_range, lasers, list(range(MC_RUNS)), indexing='ij')).T.reshape([-1,4])
    __spec__ = None
    import multiprocessing as mp
    total_runtime = params.shape[0]
    progress = mp.Value('i', 0)

    with mp.Pool(processes=MC_CORES, initializer=init_worker, initargs=(progress,total_runtime, MC_RUNS)) as pool:
        data =  np.sum(np.array(pool.map(MC_run, params)).reshape([MC_RUNS, len(lasers), len(MOT_range), len(list(Hamiltonians.keys()))]), axis = 0)
