...
def MC_run(_):
    ...
    speed, time, trans_speed, final_speed, poss = evolve_beam_vel(v0, Hamiltonians[next(samp_isotope)], laserargs={'det_slower' : -700e6/hertz_unit}, time = next(samp_time), r = next(samp_pos))
    if speed is None:
        return [None]*6
    if (speed < 0):
